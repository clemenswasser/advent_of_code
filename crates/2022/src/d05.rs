...
fn simulate_crane_ordered(stacks: &mut Vec<Vec<char>>, moves: &[&str]) {
    for line in moves {
...
fn simulate_crane(stacks: &mut Vec<Vec<char>>, moves: &[&str]) {
    for line in moves {
        let instrs: Vec<_> = line.split_ascii_whitespace().collect();
        let count: usize = instrs[1].parse().unwrap();
        let from = instrs[3].parse::<usize>().unwrap() - 1;
...
pub fn d05(input: &str) -> (String, String) {
    ...
    simulate_crane(&mut unordered_stacks, moves);
    let part1: String = unordered_stacks
        .into_iter()
        .flat_map(|stack| stack.last().copied())
        .collect();

    simulate_crane_ordered(&mut stacks, moves);
